...
public class MerchantStockController {
    ...
    public ResponseEntity addMerchantStock(@Valid @RequestBody MerchantStock MerchantStock, Errors errors){
        ...
        if (productController.getProducts().checkProductId(MerchantStock.getProductId())&& merchantController.getMerchantService().checkMerchantId(MerchantStock.getMerchantId())) {
        ...
        }else return ResponseEntity.status(400).body("Merchant ID or Product ID not found");
    }
    ...
    public ResponseEntity addProduct(@PathVariable int productid,@PathVariable int merchantid,@PathVariable int stock){
        if (merchantController.getMerchantService().checkMerchantId(merchantid) && productController.getProducts().checkProductId(productid)) {
